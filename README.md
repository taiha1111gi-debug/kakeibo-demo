...
| | 保証内容 |
|---|---|
| 実運用版 | クライアント事前フィルタ → 登録直前照合 → DB関数内チェック → **DBの一意制約**。複数端末から同時に取り込んだ場合も、最終的には一意制約により一方のみが成立する設計 |
...
タイムスタンプはUTCで保持されるため、素朴な文字列切り出しでは日本時間の月境界（1日0:00〜8:59）がひと月ずれます。月・日境界の判定を1モジュールに集約し、UTC/JST境界・閏年・締め時刻の5:59/6:00境界・datetime-localの端末タイムゾーン非依存性をユニットテストで固定しています。
...
| 領域 | 採用技術 |
|---|---|
| フロントエンド | Next.js 16（App Router・全ルート静的プリレンダリング）/ React 19 / TypeScript（strict）/ Tailwind CSS v4 |
